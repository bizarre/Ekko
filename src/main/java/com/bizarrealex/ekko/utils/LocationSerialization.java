package com.bizarrealex.ekko.utils;
...
public class LocationSerialization {
    ...
    public static Location deserializeLocation(String s) {
        Location l = new Location((World) Ekko.getInstance().getServer().getWorlds().get(0), 0.0D, 0.0D, 0.0D);
        String[] att = s.split(":");
        String[] arr$ = att;
        int len$ = att.length;
        ...
        for(int i$ = 0; i$ < len$; ++i$) {
            ...
            if(split[0].equalsIgnoreCase("@w")) {
                l.setWorld(Ekko.getInstance().getServer().getWorld(split[1]));
            }
